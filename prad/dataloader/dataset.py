...
from dataloader.load_blender import load_blender_data, load_blender_data_m
...
class Data():
    ...
    def genpc(self):
        ...
        for h_id in tqdm(range(N)):
            ...
            # for i, img in enumerate(result[:4]):
            #     img = img.detach().permute(2, 0, 1)
            ...
            # fig, ax = plt.subplots()
            # if h_id == 100:
            #     ax.imshow(np.asarray(TF.to_pil_image(img.detach())))
            #     plt.show()
            pc.append(img)
...
class Data_m():
    def __init__(self, args):
        ...
        K = None
        if args.dataset_type == 'blender':
            images, poses, render_poses, hwf, i_split = load_blender_data_m(self.datadir, args.half_res, args.testskip)
            # print('Loaded blender', images.shape, render_poses.shape, hwf, self.datadir)
            masks = images[..., -1:]
            near = 2.
            far = 6.
